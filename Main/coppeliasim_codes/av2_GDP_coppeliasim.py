...
def detect_markers(control=0):
    global x_done_flag, alignment_done, threshold_ori, \
        ...
        cam_orient_new, xadj, yadj, everything_done, ref_444_x, ref_222_x, rotation, ori_start_flag, time_c
                    ...
                    if ori_done_flag:
                        ...
                        if time_p < time_c:
                            ...
                            if x_done_flag == True and alignment_done == False:
                                ...
                                for j in range(marker_ids.size):
                                    if reference1 == marker_ids[j][0]:
                                        if round((tvec[j][0][0] + xadj), 1) > 0.5 or round((tvec[j][0][0] + xadj),
                                                                                           1) < -0.5:
                                            x_done_flag = False
                        ...
                        if y_done_flag:
                            ...
                            for k in range(marker_ids.size):
                                if reference1 == marker_ids[k][0]:
                                    if round((tvec[k][0][0] + xadj), 1) > 0.5 or round((tvec[k][0][0] + xadj),
                                                                                       1) < -0.5:
                                        x_done_flag = False
                                        y_done_flag = False
                                        alignment_done = False
